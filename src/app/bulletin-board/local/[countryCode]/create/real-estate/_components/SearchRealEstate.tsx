...
/**
 * @Description 부동산 - 방구해요에 해당하는 컴포넌트
 * @author 김영서
 **/
export default function SearchRealEstate() {
  ...
  //기간
  const [selectedTimePeriod, setSelectedTimePeriod] = useState<null | string>(null);
  const [isAnyTimeClicked, setIsAnyTimeClicked] = useState(false);
  ...
  const [isAnyBedroomClicked, setIsAnyBedroomClicked] = useState(false); //침대 수 상관없음 체크 여부

  //욕실
  const [bathroomNumber, setBathroomNumber] = useState<number|null>(null);
  const bathroomOptions = ["1", "2", "3"];
  const [isAnyBathroomClicked, setIsAnyBathroomClicked] = useState(false); //욕실 수 상관없음 체크 여부

  //면적
  const [area, setArea] = useState<number | null>(null);
  const [isAnyAreaClicked, setIsAnyAreaClicked] = useState(false); //면적 상관없음 체크 여부
  ...
  const handleAnyTimeClick = () => {
   setIsAnyTimeClicked(!isAnyTimeClicked)
    setSelectedTimePeriod("")

  };

  //침실 수 상관없음 클릭
  const handleAnyBedroomClick = () => {
    setIsAnyBedroomClicked(!isAnyBedroomClicked);
    setBedroomNumber(null);
  };

  //욕실 수 상관없음 클릭
  const handleAnyBathroomClick = () => {
    setIsAnyBathroomClicked(!isAnyBathroomClicked);
  ...
  //면적 상관없음 클릭
  const handleAnyAreaClick = () => {
    setIsAnyAreaClicked(!isAnyAreaClicked);
    setArea(null);
  };
    ...
    <form onSubmit={handleSubmit(onValid)}>
      ...
      <div className="flex flex-row gap-x-[20px]">
        ...
        <div className="flex flex-1 flex-col">
          <DropdownSelector
            selectedValue={isAnyTimeClicked ? "" : selectedTimePeriod || ""}
            ...
            trigger={trigger}
            setValue={setValue}
            disabled={isAnyTimeClicked}
          />
          <button type="button" onClick={handleAnyTimeClick}>
            <div className="flex flex-row mt-[10px] items-start justify-start">
              {isAnyTimeClicked ? (
                <>
                  {" "}
                  <CheckColorIcon color="black" />{" "}
                  <span className="text-xs ">상관없음</span>
                </>
              ) : (
                <>
                  <CheckColorIcon color="#A3A3A3" />{" "}
                  <span className="text-[#A3A3A3] text-xs ">상관없음</span>
                </>
              )}
            </div>
          </button>
      ...
      <div className=" flex flex-row gap-x-[20px]">
        ...
        <label className="block mt-[40px] text-xs text-[#808080] mb-[10px]">
          ...
          <input
            type="number"
            value={isAnyBedroomClicked ? "" : bedroomNumber || ""}
            disabled={isAnyBedroomClicked}
            onChange={(e) =>
          ...
          {/* 침실 수 상관없음 버튼 */}
          <button type="button" onClick={handleAnyBedroomClick}>
            <div className="flex flex-row mt-[10px] items-start justify-start">
              {isAnyBedroomClicked ? (
                <>
                  {" "}
        ...
        <div className="flex flex-col flex-1 w-full">
          ...
          <input
            type="number"
            value={isAnyBathroomClicked ? "" : bathroomNumber || ""}
            disabled={isAnyBathroomClicked}
            onChange={(e) =>
          ...
          {/* 욕실 수 상관없음 버튼 */}
          <button type="button" onClick={handleAnyBathroomClick}>
            <div className="flex flex-row mt-[10px] items-start justify-start">
              {isAnyBathroomClicked ? (
                <>
                  {" "}
                  <CheckColorIcon color="black" />{" "}
                  <span className="text-xs ">상관없음</span>
      ...
      <div className="flex flex-row gap-x-[20px]">
        <div className="flex flex-col flex-1 w-full relative">
          ...
          <input
            type="number"
            value={isAnyAreaClicked ? "" : area || ""}
            disabled={isAnyAreaClicked}
            onChange={(e) =>
          ...
          {/* 면적 상관없음 버튼 */}
          <button type="button" onClick={handleAnyAreaClick}>
            <div className="flex flex-row mt-[10px] items-start justify-start">
              {isAnyAreaClicked ? (
                <>
                  {" "}
 ...
 rounded-lg p-[15px]"
      ...
      <QuillEditor
        register={register}
        placeholder={"구하려는 집의 내용을 자세히 작성해주세요"}
        trigger={trigger}
        name="introduction"
        errors={errors}
        value={content}
